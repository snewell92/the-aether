...
public class ValkyrieQueen extends AbstractValkyrie implements AetherBossMob<ValkyrieQueen>, NpcDialogue, IEntityAdditionalSpawnData {
    ...
    private static final Music VALKYRIE_QUEEN_MUSIC = new Music(AetherSoundEvents.MUSIC_BOSS_VALKYRIE_QUEEN.get(), 0, 0, true);
    ...
    protected void chatWithNearby(Component message, boolean sound) {
        ...
        this.getLevel().getNearbyPlayers(NON_COMBAT, this, room).forEach(player -> this.chat(player, message, sound));
    }
    ...
    public boolean hurt(DamageSource source, float amount) {
        ...
        if (this.isReady()) {
            if (source.getEntity() instanceof LivingEntity attacker && this.getLevel().getDifficulty() != Difficulty.PEACEFUL) {
